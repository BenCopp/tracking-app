...
export default function DashboardView({ 
  ...
  const [metric, setMetric] = React.useState<'kcal' | 'p' | 'c' | 'f'>('kcal');
  ...
  const getConsistencyHistory = () => {
    ...
    if (isLongTerm) {
      // Monthly aggregation
      const months: Map<string, { kcal: number, p: number, c: number, f: number, days: number, label: string, year: number, month: number }> = new Map();
  ...
  const metricGoal = metric === 'kcal' ? userProfile.goals.kcal : userProfile.goals[metric];
  const metricLabel = metric === 'kcal' ? 'KCAL' : metric === 'p' ? 'PROT' : metric === 'c' ? 'CARB' : 'FAT';
            ...
            <div className="flex gap-2 mb-3">
              {(['kcal', 'p', 'c', 'f'] as const).map(m => (
                <button
                  ...
                  onClick={() => setMetric(m)}
                  className={cn(
                    "flex-1 py-1 px-2 rounded border font-mono text-[8px] font-bold uppercase tracking-widest transition-all",
                    metric === m 
                      ? "bg-[#3B82F6] text-white border-[#3B82F6]" 
                      : "bg-[#0A0A0B] text-[#8E9299] border-[#2A2A2E] hover:border-[#3B82F6]/50"
                  )}
                >
                  {m === 'kcal' ? 'KCAL' : m === 'p' ? 'PROT' : m === 'c' ? 'CARB' : 'FAT'}
                </button>
              ))}
            </div>
