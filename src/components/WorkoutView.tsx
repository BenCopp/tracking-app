...
export default function WorkoutView({ 
  ...
  const [selectedStepDate, setSelectedStepDate] = useState(new Date().toISOString().split('T')[0]);
  ...
  const commitSession = () => {
    ...
    // Create timestamp with current time but selected date
    const now = new Date();
    const [year, month, day] = sessionDate.split('-').map(Number);
    const stampedDate = new Date(year, month - 1, day, now.getHours(), now.getMinutes(), now.getSeconds());

    if (editingSessionId) {
      if (auth.currentUser) {
        updateDocument('workouts', editingSessionId, { 
          date: stampedDate.toISOString(), 
          exercises: sessionProgress 
        });
      } else {
        setWorkoutSessions(prev => prev.map(s => 
          s.id === editingSessionId 
            ? { ...s, date: stampedDate.toISOString(), exercises: sessionProgress }
            : s
        ));
      }
    } else {
